...
const ProductCard = ({ product }) => {
  ...
  // Image URL resolution with safety fallback
  const getImageUrl = () => {
    if (!product.image) return FALLBACK_IMAGE;
    if (product.image.startsWith('http')) return product.image;
    return `${API_IMAGE_URL}${product.image.startsWith('/') ? '' : '/'}${product.image}`;
  };
    ...
    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition bg-white flex flex-col justify-between">
      <div>
        ...
        <div className="p-4">
          ...
          <h3 className="text-lg font-bold text-gray-900 truncate">{product.title || 'Untitled Product'}</h3>
          ...
          <div className="mt-2 text-sm text-gray-600 space-y-1">
            ...
            <p><strong>MOQ:</strong> {product.moq || 1} {product.priceUnit || 'meters'}</p>
          </div>
        </div>
      </div>
