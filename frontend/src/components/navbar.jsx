...
import { ShoppingBag, Menu, X, User, ArrowRight, ShieldCheck, LogOut } from 'lucide-react';
...
export default function Navbar() {
  ...
  <div className="flex items-center gap-3">
    <Link to="/profile" className="flex items-center gap-2 text-xs font-semibold text-slate-300 hover:text-white px-3 py-2">
  ...
  <div className="flex items-center gap-3">
    ...
    <Link to="/register" className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold px-4 py-2 rounded-xl transition">
      Register
    </Link>
  </div>
)}
    ...
    <nav className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden md:flex items-center gap-8 font-medium text-slate-600 text-sm">
            <Link to="/" className="text-slate-900 font-semibold hover:text-indigo-600 transition">Marketplace</Link>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-b border-slate-100 px-6 py-6 space-y-4 animate-in slide-in-from-top duration-200">
          <Link to="/" className="block text-slate-800 font-semibold py-1">Marketplace</Link>
          <a href="#categories" className="block text-slate-600 py-1">Categories</a>
          <Link to="/supplier/dashboard" className="block text-indigo-600 font-semibold py-1">Supplier Portal</Link>
